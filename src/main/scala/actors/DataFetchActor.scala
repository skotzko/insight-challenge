...
class DataFetchActor(val cleanup: ActorRef) extends ActorBase {
  ...
  /**
    * Loads tweets from the batch file and sends on for processing.
    */
  ...
  /**
    ...
    */
  def loadTweets() = {
    // TODO: implement actual loading of tweets from file in efficient manner
    // load in batch? then loop over? cut up the loading?
