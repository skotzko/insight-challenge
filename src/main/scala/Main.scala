...
// takes in raw json text and cleans it
class CleanupActor(val processor: ActorRef) extends Actor {

  def printAvg(x: String) = println("this is the average\n")
  ...
  def receive = {
    ...
    case x if x.toString.startsWith("{\"limit\":") => println("rate limit! ignoring\n")

    // otherwise process & send on
...
//    case
  ...
  def checkIfShouldTerminate = {
    ...
    numberOfChildren == 0) context.stop(self)
  }
